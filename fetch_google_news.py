#!/usr/bin/env python3
"""
Google News RSS Fetcher for Bangladesh News
Fetches news from Google News RSS feeds focused on Bangladesh topics
...
# Google News RSS feeds for Bangladesh - LATEST NEWS ONLY (past 24 hours)
# Adding when:1d parameter to get only news from last 24 hours
# Includes both English and Bangla (বাংলা) language feeds
GOOGLE_NEWS_FEEDS = [
    # === English Feeds ===
    {
        "name": "Google News - Bangladesh (Latest) [EN]",
        "url": "https://news.google.com/rss/search?q=Bangladesh+when:1d&hl=en-BD&gl=BD&ceid=BD:en",
        "language": "en"
    },
    {
        "name": "Google News - Bangladesh Politics (Latest) [EN]",
        "url": "https://news.google.com/rss/search?q=Bangladesh+politics+when:1d&hl=en-BD&gl=BD&ceid=BD:en",
        "language": "en"
    },
    {
        "name": "Google News - Bangladesh Cricket (Latest) [EN]",
        "url": "https://news.google.com/rss/search?q=Bangladesh+cricket+when:1d&hl=en-BD&gl=BD&ceid=BD:en",
        "language": "en"
    },
    {
        "name": "Google News - Dhaka (Latest) [EN]",
        "url": "https://news.google.com/rss/search?q=Dhaka+when:1d&hl=en-BD&gl=BD&ceid=BD:en",
        "language": "en"
    },
    {
        "name": "Google News - Bangladesh Economy (Latest) [EN]",
...
def save_articles(all_articles: Dict[str, List[Dict]], output_dir: Path):
    ...
    with open(txt_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"English + Bangla (ইংরেজি + বাংলা) Coverage\n")
...
def main():
    ...
    print("📰 Languages: English + Bangla (ইংরেজি + বাংলা)\n")
    
    # Create output directory
    now = datetime.now()
    date_folder = now.strftime("%Y-%m-%d")
    output_dir = Path("output_google_news") / date_folder
